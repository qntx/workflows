...
| `publish-npm-bun.yml@main`    | `publish-npm.yml@<sha>`       | **`package-manager: bun`**. `bun-version` default `'latest'` → `'1.4'` (do not pass `latest`).                        | OIDC: must add `id-token: write`.                                                    | `publish / Build & Publish` → `publish / publish`                                                     |
