...
| `publish-npm.yml@main`        | `publish-npm.yml@v2.0.0`       | Unchanged filename. OIDC callers must grant `id-token: write`.                                                        | OIDC: `contents: read` + `id-token: write`. Token: `contents: read`.                 | `publish / Build & Publish` → `publish / publish`                                                     |
